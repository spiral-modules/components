...
class DocumentSchema extends ReflectionEntity
{
     ...
     */
    public function __construct($class, SchemaBuilder $builder)
    {
    ...
    public function getMutators()
    {
        ...
        foreach ($this->getFields() as $field => $type) {
            ...
            if (isset($resolved[AbstractEntity::MUTATOR_ACCESSOR])) {
                //Accessor options include field type, this is ODM specific behaviour
                $resolved[AbstractEntity::MUTATOR_ACCESSOR] = [
                    $resolved[AbstractEntity::MUTATOR_ACCESSOR],
    ...
    {
        if ($document->getDatabase() != $this->getDatabase()) {
            return false;
        }
    ...
    private function normalizeMutators(array $mutators)
    {
        foreach ($mutators as $mutator => &$filters) {
            foreach ($filters as $field => $filter) {
                ...
                if ($mutator == AbstractEntity::MUTATOR_ACCESSOR && is_string($filters[$field])) {
                    $type = null;
